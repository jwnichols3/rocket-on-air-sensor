...
## 2026-08-31 - 0.7.1 -> 0.7.2 (preset art changed)

- **Marker used: THE PRESET PICKER ITSELF.** 0.7.2 only changed how presets look, so
  `Buttons` -> `Presets` -> the connection is where the change lives: the `Panel` row went from
  moon / sun / moon to moon-grey / sun-grey / sun-white. Art in the picker is generated by the
  RUNNING instance, so it cannot be stale - it is a marker for any release that touches preset
  appearance, and it needs no variable and no count.
- **And it is the marker that most needs step 5 said out loud.** The picker updated; Rocket's
  PLACED key did not, because a placed button is a one-time copy. A run that stops at "the
  picker looks right" reports success for a deck that still shows the old face. Check both, and
  say which one the human has to re-drag.
- Loop outcome: **one addition** (preset art as a marker, and its trap). The
  first-click-after-navigate rule earned its place again - the pencil ate two clicks this run.
