...
<h3>4.3b <code>POST /cycle</code> - one button that walks the table</h3>
<p>For a Stream Deck key that replaces four of them. Each press moves one stop around a ring of rows and wraps at the end, so you press until the row you want comes up.</p>
<pre><code>curl -sS -X POST 'http://127.0.0.1:8484/cycle?ring=available,on-air,interruptible&amp;source=companion'</code></pre>
<p>Omit <code>?ring=</code> and it is every row except <code>unknown</code>, in table order. The response is the usual <code>GET /status</code> body, so the row you landed on is in <code>state</code>.</p>
<p><strong>Do not implement this yourself by reading <code>state</code> and writing its successor.</strong> It works until somebody presses twice quickly - the second press starts before the first one's answer arrives, reads the same <code>state</code>, and writes the same row - and pressing quickly is exactly how this button is used. The server computes the successor inside its write queue, where each press can see the one before it.</p>
<p>Two behaviours worth knowing. A current state the ring does not name (<code>unknown</code> at boot) goes to the <strong>first</strong> entry rather than erroring. And a ring entry this server does not have is <strong>dropped</strong>, not refused, because a Companion button freezes its options the day it is placed and a deleted row would otherwise brick it forever; only a ring with nothing valid in it gets a <code>400</code>.</p>
...
<p>Three things end a sleep: <code>POST /panel/wake</code>, the panel's own scheduled wake time, and <strong>a <code>human:</code> write that changes the row while the panel is dark</strong>. That last one is there because darkening the glass and then pressing AVAILABLE means you want to see AVAILABLE. An <code>auto:</code> write does not do it - the detector heartbeats, and a manual sleep has to outlive the next Zoom call - and neither does re-asserting the row already held.</p>
<p><strong>A busy row is not one of them, and this is the part that will surprise you.</strong> The panel refuses to <em>darken</em> while the current row is busy, however it was asked, so a sleep pressed during a call appears to do nothing - but the switch is on, and the sleep is <em>pending</em>. It takes effect the moment the row stops being busy. <code>delivered:true</code> with the panel still lit is correct, not an error; the panel's own <code>Night</code> sensor says <code>lit (sleep pending - busy or no data)</code> while this is true.</p>
